...
export default function Header({ habitsDone, totalHabits, tasksDone, totalTasks, projectsDone, totalProjects, user }) {
  const [displayPct, setDisplayPct] = useState(user.levelPct || 0);
  ...
  useEffect(() => {
    if (user.level > prevLevel.current) {
      // LEVEL UP SEQUENCE (Fill -> Snap 0% -> New Pct)
      setDisplayPct(100);
      const timer1 = setTimeout(() => {
        ...
        const timer2 = setTimeout(() => {
          ...
          setDisplayPct(user.levelPct);
          prevLevel.current = user.level;
        }, 50);
        return () => clearTimeout(timer2);
      }, 600);
      return () => clearTimeout(timer1);
    } else if (user.level < prevLevel.current) {
      // LEVEL DOWN SEQUENCE (Drain 0% -> Snap 100% -> New Pct)
      setDisplayPct(0);
      const timer1 = setTimeout(() => {
        setTransitionDisabled(true);
        setDisplayPct(100);
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, width: '100%', marginTop: -2 }}>
        <div className="progress-bar" style={{ flex: 1, height: 3.5 }}>
          <div className="progress-bar-fill" style={{ 
            width: `${displayPct}%`,
            transition: transitionDisabled ? 'none' : 'width 0.6s cubic-bezier(0.4, 0, 0.2, 1)'
          }} />
        </div>
